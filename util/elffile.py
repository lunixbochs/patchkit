...
class ElfFile(StructBase):
    ...
    def _pack_program_data(self, block, offset=0):
        """
        Pack the program header data. As a side effect, set the offset
        and in the program headers.
        """
        ...
        for ph in self.progs:
            ...
            for phd in self.progs:
                if PT.bycode[prog.type] == PT.byname['PT_LOAD']:
                    continue
    ...
    def _pack_section_data(self, block, offset=0):
        """
        Pack the section header data. As a side effect, set the offset
    ...
    def read(self, vaddr, size):
        ...
        progs = [p for p in reversed(self.progs) if vaddr in p and PT.bycode[p.type] == PT.byname['PT_LOAD']]
        if progs:
            ph = progs[0]
            off = vaddr - ph.vaddr
